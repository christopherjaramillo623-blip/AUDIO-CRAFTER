...
do
    ...
    tagsBtn.MouseButton1Click:Connect(function()
        ...
        -- Hide/show ALL AC billboards on ALL players including local player's own tag
        for _,p in ipairs(AC.Players:GetPlayers()) do
            if p.Character then
                local h=p.Character:FindFirstChild("Head")
                if h then
                    for _,bb in ipairs(h:GetChildren()) do
                        if bb:IsA("BillboardGui") and (bb.Name=="AC_Billboard" or bb.Name=="AC_Billboard_Logo") then
                            -- For logo billboard only enable if we're far AND tags are on
                            -- For now just track enabled state; distance loop will correct logo
                            if bb.Name=="AC_Billboard" then bb.Enabled=AC.tagsVisible end
                            if bb.Name=="AC_Billboard_Logo" then bb.Enabled=false end -- distance loop handles this
                        end
                    end
                end
            end
...
do
    ...
    local function notifyACUser(p)
        ...
        local dispName=(p.DisplayName and p.DisplayName~=p.Name) and p.DisplayName or ""

        local NOTIF_W=260; local NOTIF_H=60
        local yOff=_notifOffset; _notifOffset=_notifOffset+NOTIF_H+8

        local f=Instance.new("Frame",AC.screenGui)
        f.Size=UDim2.new(0,NOTIF_W,0,NOTIF_H)
        f.BackgroundColor3=bgColor
        f.BackgroundTransparency=0.35  -- more transparent like AK style
        f.ZIndex=300; f.ClipsDescendants=false
        ...
        pcall(function()
            local imgUrl,_=AC.Players:GetUserThumbnailAsync(p.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size48x48)
            thumb.Image=imgUrl
        end)
        ...
        tpBtn.MouseButton1Click:Connect(function()
            if p.Character then
                ...
                if r and m then
                    ...
                    local leftCF=r.CFrame*CFrame.new(-3,0,0)
                    local rightBlocked=false
                    for _,pl in ipairs(AC.Players:GetPlayers()) do
                        if pl~=AC.player and pl~=p and pl.Character then
                    ...
                    m.CFrame=rightBlocked and leftCF or rightCF
                    AC.toast("TP'd to "..p.Name,AC.PUR_BRIGHT)
                end
            else
                AC.toast(p.Name.." not spawned yet",AC.ORANGE_W)
            end
        end)

        -- Slide in from RIGHT, position TOP RIGHT stacked
        ...
        task.delay(8,function()
            if f and f.Parent then
                AC.TS:Create(f,TweenInfo.new(0.25,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Position=UDim2.new(1,10,0,yOff)}):Play()
                task.delay(0.3,function()
                    if f and f.Parent then f:Destroy() end
                    _notifOffset=math.max(10,_notifOffset-(NOTIF_H+8))
                end)
            end
        end)
    end
    ...
    -- Leave notification for AC users
    AC.Players.PlayerRemoving:Connect(function(p)
        ...
        local accentColor=pIsOwner and Color3.fromRGB(255,200,40) or AC.PUR_BRIGHT
        local bgColor=pIsOwner and Color3.fromRGB(18,10,0) or Color3.fromRGB(8,2,18)
        local NOTIF_W=260; local NOTIF_H=46
        local yOff=_notifOffset; _notifOffset=_notifOffset+NOTIF_H+8
        local f=Instance.new("Frame",AC.screenGui)
        f.Size=UDim2.new(0,NOTIF_W,0,NOTIF_H)
        f.BackgroundColor3=bgColor; f.BackgroundTransparency=0.35; f.ZIndex=300; f.ClipsDescendants=false
        Instance.new("UICorner",f).CornerRadius=UDim.new(0,10)
        local stroke=Instance.new("UIStroke",f); stroke.Color=accentColor; stroke.Thickness=1.2; stroke.Transparency=0.4
        local bar=Instance.new("Frame",f); bar.Size=UDim2.new(0,3,1,-8); bar.Position=UDim2.new(0,0,0,4); bar.BackgroundColor3=accentColor; bar.BorderSizePixel=0; bar.ZIndex=301; Instance.new("UICorner",bar).CornerRadius=UDim.new(1,0)
